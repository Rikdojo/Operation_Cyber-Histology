...
class LightAlexNet(nn.Module):
    """Small CNN used as the lightweight AlexNet comparison point."""
    def __init__(self, in_channels, num_classes, **kwargs):
        super().__init__()
...
class LightVGG16(nn.Module):
    """Reduced VGG-style model with fewer channels and blocks."""
    def __init__(self, in_channels, num_classes, **kwargs):
        super().__init__()
        drop_rate = kwargs.get("drop_rate", 0.3)

        self.features = nn.Sequential(
            VGGBlock(in_channels, 32, num_convs=1),
            VGGBlock(32, 64, num_convs=1),
            VGGBlock(64, 128, num_convs=2),
            nn.AdaptiveAvgPool2d((1, 1)),
        )
        self.classifier = nn.Sequential(
...
class LightResNet18(nn.Module):
    """Reduced residual model for efficient Task 2 comparisons."""
    def __init__(self, in_channels, num_classes, **kwargs):
        super().__init__()
        activation = getattr(nn, kwargs.get("activation_str", activation_str) or activation_str)
        ...
        self.activation = activation(inplace=True)
        self.stage1 = ResBlock(32, 32, activation(inplace=True), stride=1)
        self.stage2 = ResBlock(32, 64, activation(inplace=True), stride=2)
        self.stage3 = ResBlock(64, 128, activation(inplace=True), stride=2)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
