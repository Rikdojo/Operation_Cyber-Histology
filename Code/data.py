...
def get_loaders(data, data_path, batch_size, val_split=0.1):
    d_path = Path(data_path) / f"{data}.pt" # changed  f"{data}_data.pt to f"{data}.pt", because it does not match the file name in the data folder. 
    ...
    mean = train_data.mean() # normalize the image data with z-score normalization 
    std = train_data.std()
